...
impl S3CrtClient {
    pub(super) async fn put_object(
        ...
        let (mpu_created_sender, mpu_created) = oneshot::channel::<Result<(), S3RequestError>>();
        ...
        let body = {
            ...
            // Before returning, we need to await for the multi-part upload to be created, so we can report errors.
