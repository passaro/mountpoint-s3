...
impl<'a> MetaRequestOptions<'a> {
    ...
    pub fn message(&mut self, message: Message<'a>) -> &mut Self {
        // SAFETY: we aren't moving out of the struct.
        let options = unsafe { Pin::get_unchecked_mut(Pin::as_mut(&mut self.0)) };
        let message = options.message.insert(message);
        options.inner.message = message.inner.as_ptr();

        if let Some(content) = message.body_contents {
            ...
            options.inner.request_body = unsafe { content.as_aws_byte_cursor() };
        }
        self
    }
