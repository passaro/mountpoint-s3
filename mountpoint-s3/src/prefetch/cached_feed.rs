...
use crate::prefetch::{
    feed::{ObjectPartFeed, RequestRange, TaskError},
    part::Part,
    part_queue::PartQueueProducer,
    task::{PrefetchReadError, RequestTask},
};
...
#[derive(Debug)]
pub struct CachedPartFeed<Client, Cache, Runtime> {
    inner: CachedPartFeedInner<Client, Cache>,
    runtime: Runtime,
}

impl<Client, Cache, Runtime> CachedPartFeed<Client, Cache, Runtime> {
    pub fn new(client: Arc<Client>, runtime: Runtime, cache: Cache) -> Self {
        Self {
            inner: CachedPartFeedInner {
                client,
                cache: Arc::new(cache),
            },
            runtime,
...
impl<Client, Cache, Runtime> ObjectPartFeed<Client, Runtime> for CachedPartFeed<Client, Cache, Runtime>
where
    Client: ObjectClient + Send + Sync + 'static,
    Cache: DataCache<CacheKey> + Send + Sync + 'static,
    Runtime: Spawn,
{
...
#[derive(Debug)]
struct CachedPartFeedInner<Client, Cache> {
    client: Arc<Client>,
    cache: Arc<Cache>,
}

impl<Client, Cache> Clone for CachedPartFeedInner<Client, Cache> {
    fn clone(&self) -> Self {
        Self {
...
impl<Client, Cache> CachedPartFeedInner<Client, Cache>
where
    Client: ObjectClient + Send + Sync + 'static,
    Cache: DataCache<CacheKey> + Send + Sync,
{
    async fn get_from_cache(
        ...
        if_match: ETag,
        range: RequestRange,
        part_queue_producer: PartQueueProducer<TaskError<Client>>,
    ) {
        let block_size = self.cache.block_size();
        let block_range = block_indices_for_byte_range(&range, block_size);

        let cache_key = (key.to_owned(), if_match);
                ...
                Some(Ok((offset, body))) => {
                    ...
                    while !body.is_empty() {
                        ...
                        if let Err(e) = buffer.extend(chunk.into()) {
                            ...
                            part_queue_producer.push(Err(PrefetchReadError::Integrity(e)));
                            return;
                        }
...
mod tests {
    ...
    fn test_read_from_cache(
        ...
        let feed = CachedPartFeed::new(mock_client.clone(), runtime, cache);
        ...
        let first_read_count = {
            ...
            let request_task = feed.spawn_get_object_request(bucket, key, etag.clone(), range, 0);
            compare_read(key, seed, request_task);
    ...
    fn compare_read<E: std::error::Error + Send + Sync>(key: &str, seed: u8, mut request_task: RequestTask<E>) {
