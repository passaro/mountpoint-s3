...
pub fn default_prefetch<Runtime>(
    runtime: Runtime,
    prefetcher_config: PrefetcherConfig,
) -> Prefetcher<ClientPartStream<Runtime>>
where
...
pub fn cached_prefetch<Cache, Runtime>(
    cache: Cache,
    runtime: Runtime,
    prefetcher_config: PrefetcherConfig,
) -> Prefetcher<CachedPartStream<Cache, Runtime>>
where
    Cache: DataCache + Send + Sync + 'static,
    Runtime: Spawn + Send + Sync + 'static,
{
    let part_stream = CachedPartStream::new(runtime, cache);
    Prefetcher::new(part_stream, prefetcher_config)
}
...
mod tests {
    ...
    fn no_cache_stream() -> ClientPartStream<ThreadPool> {
        let runtime = ThreadPool::builder().pool_size(1).create().unwrap();
        ClientPartStream::new(runtime)
    }

    fn cache_stream(block_size: usize) -> CachedPartStream<InMemoryDataCache, ThreadPool> {
        let runtime = ThreadPool::builder().pool_size(1).create().unwrap();
        let cache = InMemoryDataCache::new(block_size as u64);
        CachedPartStream::new(runtime, cache)
    }
    ...
    fn sequential_read_small(data_cache_config: DataCacheConfig) {
        ...
        match data_cache_config {
            DataCacheConfig::NoCache => {
                run_sequential_read_test(no_cache_stream(), 1024 * 1024 + 111, 1024 * 1024, config);
            }
            DataCacheConfig::InMemoryCache { block_size } => {
                run_sequential_read_test(cache_stream(block_size), 1024 * 1024 + 111, 1024 * 1024, config);
            }
        }
    }
    ...
    fn sequential_read_medium(data_cache_config: DataCacheConfig) {
        ...
        match data_cache_config {
            DataCacheConfig::NoCache => {
                run_sequential_read_test(no_cache_stream(), 16 * 1024 * 1024 + 111, 1024 * 1024, config);
            }
            DataCacheConfig::InMemoryCache { block_size } => {
                run_sequential_read_test(cache_stream(block_size), 16 * 1024 * 1024 + 111, 1024 * 1024, config);
            }
        }
    }
    ...
    fn sequential_read_large(data_cache_config: DataCacheConfig) {
        ...
        match data_cache_config {
            DataCacheConfig::NoCache => {
                run_sequential_read_test(no_cache_stream(), 256 * 1024 * 1024 + 111, 1024 * 1024, config);
            }
            DataCacheConfig::InMemoryCache { block_size } => {
                run_sequential_read_test(cache_stream(block_size), 256 * 1024 * 1024 + 111, 1024 * 1024, config);
            }
        }
    }
    ...
    fn fail_request_sequential_small(err_value: &str, data_cache_config: DataCacheConfig) {
        ...
        match data_cache_config {
            DataCacheConfig::NoCache => {
                fail_sequential_read_test(no_cache_stream(), 1024 * 1024 + 111, 1024 * 1024, config, get_failures);
            }
            DataCacheConfig::InMemoryCache { block_size } => {
                fail_sequential_read_test(
                    cache_stream(block_size),
                    1024 * 1024 + 111,
                    1024 * 1024,
                    config,
                    get_failures,
    ...
    proptest! {
        ...
        fn proptest_sequential_read(
            ...
            match config.data_cache_config {
                DataCacheConfig::NoCache => {
                    run_sequential_read_test(no_cache_stream(), size, read_size, config);
                },
                DataCacheConfig::InMemoryCache { block_size } => {
                    run_sequential_read_test(cache_stream(block_size), size, read_size, config);
                },
            }
        }
    ...
    fn test_sequential_read_regression() {
        ...
        run_sequential_read_test(no_cache_stream(), object_size, read_size, config);
    }
    ...
    proptest! {
        ...
        fn proptest_random_read(
            ...
            match config.data_cache_config {
                DataCacheConfig::NoCache => {
                    run_random_read_test(no_cache_stream(), object_size, reads, config);
                },
                DataCacheConfig::InMemoryCache { block_size } => {
                    run_random_read_test(cache_stream(block_size), object_size, reads, config);
                },
            }
        }
    }
    ...
    fn test_forward_seek(first_read_size: usize, part_size: usize) {
        ...
        let prefetcher = Prefetcher::new(no_cache_stream(), prefetcher_config);
